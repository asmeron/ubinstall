...
#### 4. Извлечь данные: дата окончания лицензии, тип лицензии, статус активации, персональный идентификатор;
...
#### 6. Если статус активации равен "unactivated", формируем ответ клиенту, в котором содержится отказ;
В интерфейсе инсталлятора выводится сообщение о необходимости прохождения процедуры активации системы на хосте для получения обновлений и поддержки.
#### 6. Если дата окончания не пройдена, проверяем, какие типы лицензий доступны;
Доступные типы лицензии берутся из значений "type".
#### 7. Формируем каталог репозитория и ответ клиенту, в котором содержатся данные для доступа к репозиторию.
Алгоритм:
